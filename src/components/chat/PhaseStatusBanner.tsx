    ...
    return (
      ...
      >
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          ...
          <p className="text-sm text-violet-300">Đang tạo Cursor Prompts... (15–30 giây)</p>
        </div>
      </div>
    )
  }
      ...
      <div className="px-4 py-3 bg-violet-950/50 border-t border-violet-800/50 flex-shrink-0">
        <div className="max-w-3xl mx-auto flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-violet-400 mt-0.5">Nhấn Generate để nhận bộ prompt từng bước cho Cursor</p>
          </div>
          <Button
            onClick={onGenerate}
            aria-label="Tạo Cursor Prompts"
  ...
  if (phase === 'complete' && outputDocs) {
    return (
      <div className="px-4 py-3 bg-green-950/30 border-t border-green-800/30 flex-shrink-0" role="status">
        <div className="max-w-3xl mx-auto flex items-center justify-between gap-4">
          <p className="text-sm text-green-300">✅ Cursor Prompts đã sẵn sàng — xem ở panel bên phải</p>
          <button
            onClick={onGenerate}
